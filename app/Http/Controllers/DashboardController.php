...
class DashboardController extends AccountBaseController
{
    ...
    public function filter(Request $request)
    {
        ...
        // $totalavoidCost = DB::table('users')
        //     ->join('projects', 'users.id', '=', 'projects.pm_id')
        //     ->join('project_milestones', 'projects.id', '=', 'project_milestones.project_id')
        //     ->join('payments', 'project_milestones.invoice_id', '=', 'payments.invoice_id')
        //     ->whereBetween('project_milestones.created_at', [$afterAssignStartDate, $releaseEndDate])
        //     // ->whereNotNull('project_milestones.invoice_id')
        //     ->whereBetween('payments.paid_on', [$afterAssignStartDate, $releaseEndDate])
        //     ->where('users.id', $pmId)
        //     ->sum('project_milestones.cost');
        ...
        // $assignMilestonesCostCountWithoutComplete = $query->sum('cost');

        // $assignMilestonesRowCountWithoutComplete = $query->count();

        // $totalAssignMilestonesRow = $assignMilestonesRowCountWithoutComplete + $totalCompleteMilestonesRow; //rowwise 

        // $totalAssignMilestonesCost = $assignMilestonesCostCountWithoutComplete + $totalCompleteMilestonesCost; //valuewise 
        ...
        // $query = Project::join('tasks', 'projects.id', '=', 'tasks.project_id')
        //     ->whereNull('tasks.subtask_id')
        //     ->where('tasks.start_date', '<', $startDate)
        //     ->whereBetween('tasks.updated_at', [$startDate, $releaseEndDate])
        //     ->where('tasks.board_column_id', '4')
        //     ->where('projects.pm_id', $pmId);

        // $beforeAssignTasksRow = $query->count();

        // $totalAssignTasksRow += $beforeAssignTasksRow;
        ...
        //View project history by project manager//
        ...
        return view('dashboard.employee.pm_dashboard', compact('username', 'startDate', 'assignEndDate', 'averageProjectCompletionTime', 'RunningtotalCompleteTasksRow', 'RunningtotalAssignTasksRow', 'RunningtaskCompletionRate', 'RunningtotalCompleteMilestonesCost', 'RunningtotalAssignMilestonesCost', 'RunningmilestoneCompletionRatebyCost', 'RunningtotalAssignMilestonesRow', 'RunningtotalCompleteMilestonesRow', 'RunningmilestoneCompletionRatebyRow', 'projectsCompletionRatebyRowWithoutQC', 'ProjectCompletionRatebyCostWithoutQC', 'ProjectCompletionWithoutQCCountbyCost', 'ProjectCompletionWithoutQCCountbyRow', 'ProjectCompletionWithoutQCCountbyRow', 'dataviewforProject', 'RunningProjectCompletionRatebyCost', 'RunningProjectsCompletionRatebyRow', 'totalRunningCompleteProjectsSum', 'totalRunningCompleteProjectsRow', 'totalRunningAssignProjectsSum', 'totalRunningAssignProjectsRow', 'totalAssignProjectsRow', 'totalAssignProjectsSum', 'ProjectCompletionRatebyCost', 'projectsCompletionRatebyRow', 'totalCompleteProjectsSum', 'totalCompleteProjectsRow', 'totalCompleteTasksRow', 'totalAssignTasksRow', 'taskCompletionRate', 'totalCompleteMilestonesCost', 'totalAssignMilestonesCost', 'milestoneCompletionRatebyCost', 'dataview', 'totalAssignMilestonesRow', 'totalCompleteMilestonesRow', 'milestoneCompletionRatebyRow', 'datafortasks'));
    }
    ...
    public function taskCompletionRate(Request $request)
    {
        ...
        $totalavoidrow = DB::table('projects')
            ->join('tasks', 'projects.id', '=', 'tasks.project_id')
            ->whereBetween('tasks.start_date', [$afterAssignStartDate, $releaseEndDate])
            ->whereBetween('tasks.updated_at', [$afterAssignStartDate, $releaseEndDate])
            ->whereNull('tasks.subtask_id')
            ->where('tasks.board_column_id', '4')
            ->where('projects.pm_id', $pmId)
            ->count();
        ...
        $totalCompleteTasksRow = $totalrow - $totalavoidrow;
        ...
        $query = Project::join('tasks', 'projects.id', '=', 'tasks.project_id')
            ...
            ->whereBetween('tasks.start_date', [$startDate, $assignEndDate])
            //  why not working 
            // ->orWhere(function ($query) use ($startDate, $releaseEndDate, $afterAssignStartDate) {
            //     $query->WhereBetween('tasks.updated_at', [$startDate, $releaseEndDate])
            //         ->WhereNotBetween('tasks.start_date', [$afterAssignStartDate, $releaseEndDate])
            //         ->where('board_column_id', '4');
            // })
            ->where('projects.pm_id', $pmId);

        $totalAssignTasksRow = $query->count();
        ...
        $datafortasks = Project::select('projects.pm_id', 'tasks.id', 'tasks.heading', 'projects.project_name', 'tasks.board_column_id', 'tasks.start_date', 'tasks.updated_at')
            ...
            ->whereNull('tasks.subtask_id')
            ->where('projects.pm_id', $pmId)
            ->Where(function ($query) use ($startDate, $assignEndDate, $releaseEndDate) {
                $query->whereBetween('tasks.start_date', [$startDate, $assignEndDate])
                    ->orWhere(function ($query) use ($startDate, $releaseEndDate) {
                        $query->WhereBetween('tasks.updated_at', [$startDate, $releaseEndDate])
                            ->where('tasks.start_date', '<', $startDate)
                            ->where('board_column_id', '4');
                    });
        ...
        return view('dashboard.employee.pm_dashboard', compact('startDate', 'assignEndDate', 'totalCompleteTasksRow', 'totalAssignTasksRow', 'taskCompletionRate', 'datafortasks'));
    }
}
